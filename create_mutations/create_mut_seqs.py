...
# create a FASTA file to input into Clustal Omega but using file cftr_sequences.fasta
import textwrap
wrapper_count = textwrap.TextWrapper(width=60)

# print(just_sequences)
with open("create_mutations/cftr_sequences.fasta", "w") as create_fasta:
    for index, row in just_sequences.iterrows():
        name = row["HGVS name (NM_000492.3)"]
        sequence = row["mut_DNA"]

        create_fasta.write(f">{name}\n") # write the first line as identifier for each sequence
        wrapped_seq_list = wrapper_count.wrap(text=sequence)
        wrapped_seq = "\n".join(wrapped_seq_list)
